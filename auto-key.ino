...
// menu 3
#define MENU3_LEN 3
// labels may be no longer then 27 characters and are all lower case keys
#define MENU3_LABEL_LEN 27
const PROGMEM uint8_t menu3_labels [MENU3_LEN][MENU3_LABEL_LEN] = {
  // credentials
...
// keys may be no longer then 235 sequences and consist of a key press and a modifier value (2 bytes for 235 sequences = 470 bytes)
...
const PROGMEM uint8_t menu3_keys [MENU3_LEN][MENU3_KEY_LEN] = {
  // credentials
  {
...
// key mappings en-US/de-CH
#define MAPPING1_LEN 22
const PROGMEM uint8_t mapping1 [MAPPING1_LEN] = {
  0x26, 0x25,
  0x27, 0x26,
  0x1e, 0x30,
  0x24, 0x23,
...
#define MAPPING2_LEN 27
const PROGMEM uint8_t mapping2 [MAPPING2_LEN] = {
  0x1f, 0x1f, 0x40,
  0x20, 0x20, 0x40,
  0x21, 0x31, 0x0,
  0x23, 0x2e, 0x0,
...
#define MAPPING3_LEN 27
const PROGMEM uint8_t mapping3 [MAPPING3_LEN] = {
  0x2d, 0x38, 0x0,
  0x2e, 0x27, 0x2,
  0x2f, 0x2f, 0x40,
  0x30, 0x30, 0x40,
...
void loop() {
  switch(state) {
    ...
    case 2:
      ...
      if (menu2_count == PIN_LEN) {
        ++state;
        menu_delete(MENU2_LEN + SEPARATOR_LEN + menu2_count);
        menu3_print();
      }
      break;
...
void menu3_print() {
    uint16_t mem_address = ((uint16_t) menu3_labels) + (menu3_selection * MENU3_LABEL_LEN);
    uint8_t i = 0;
    for (; i < MENU3_LABEL_LEN; ++i) {
      uint8_t key = pgm_read_byte_near(mem_address + i);
      if (key > 0) {
        send_key_stroke(key, 0);
      } else {
        break;
      }
    }
...
void menu3_selection_print() {
    uint16_t mem_address = ((uint16_t) menu3_keys) + (menu3_selection * MENU3_KEY_LEN);
...
void menu_separator_print() {
  // [:] in de-CH or en-US
  send_key_stroke(0x33, MOD_SHIFT_LEFT);
  // [space]
...
void send_key_stroke(byte keyPress, byte modifiers) {
  if (!lang) {
    DigiKeyboard.sendKeyStroke(keyPress, modifiers);
    return;
  }
  if (keyPress == 0x1c) {
    keyPress = 0x1d;
    ...
    keyPress = 0x1c;
  } else if (modifiers == 0x2) {
    for (uint8_t i = 0; i < MAPPING1_LEN; i += 2) {
      if (keyPress == pgm_read_byte_near(mapping1 + i)) {
        keyPress = pgm_read_byte_near(mapping1 + i + 1);
        DigiKeyboard.sendKeyStroke(keyPress, modifiers);
        return;
      }
    }
    for (uint8_t i = 0; i < MAPPING2_LEN; i += 3) {
      if (keyPress == pgm_read_byte_near(mapping2 + i)) {
        keyPress = pgm_read_byte_near(mapping2 + i + 1);
        modifiers = pgm_read_byte_near(mapping2 + i + 2);
        break;
      }
    }
  } else if (modifiers == 0x0) {
    for (uint8_t i = 0; i < MAPPING3_LEN; i += 3) {
      if (keyPress == pgm_read_byte_near(mapping3 + i)) {
        keyPress = pgm_read_byte_near(mapping3 + i + 1);
        modifiers = pgm_read_byte_near(mapping3 + i + 2);
        break;
      }
    }
  }
